...
## NOW USE THE VEGAN PACKAGE DIVERSITY FUNCTION
## Shannon diversity function for niche breadth 
# Shannon diversity index function 
#shannon<-function(x){
  log<-log(x/sum(x))
  is.na(log) <- sapply(log, is.infinite) ### replace infinite values with 0s
...
###########################################################################
pdf("Results/16.12.21.Results_2/Niche_overlap_clade_8_rep1.pdf", width=8, height=10)
...
# B. Real Breeding + hypo wintering
  ...
  # Alternative Breadth
  # Put Breeding and wintering occurences together and grid the whole thing together
  # PCA SCORES FOR ANNUAL BACKGROUND
  # PCA SCORES FOR ANNUAL OCCURENCE
  scores.brwi<-rbind(real.scores.br,real.scores.wi)   
  # Calculate the Occurence Densities Grid with ecospat.grid.clim.dyn 
  # This is potentially problematic- as global is the same as the species bg
  X1<-ecospat.grid.clim.dyn(real.clim.brwi,real.clim.brwi,scores.brwi,R=100)

  # Calculate annual niche breadth from these gridding values
  An_breadth<-X1$z.unco@data@values
  An_breadth<-diversity(An_breadth,index = "shannon")
  ...
  resds <- data.frame(Species=splist[i], Comparison="real", D.obs=a$obs$D, eq.P=a$p.D,eq.mean.sim=mean(a$sim$D), eq.se.sim=se(a$sim$D), si12.P=b$p.D, si12.mean.sim=mean(b$sim$D), si12.se.sim=se(b$sim$D), si21.P=b2$p.D, si21.mean.sim=mean(b2$sim$D), si21.se.sim=se(b2$sim$D), an_breadth=An_breadth, Niche_expansion=unname(filling.i$dynamic.index.w[1]),Niche_stability=unname(filling.i$dynamic.index.w[2]), Niche_unfilling=unname(filling.i$dynamic.index.w[3]))
  ...
  if (resmig[i]=="mig") {
    ...
    tmp <- data.frame(Species=splist[i], Comparison="hypo_br", D.obs=a$obs$D, eq.P=a$p.D, eq.mean.sim=mean(a$sim$D), eq.se.sim=se(a$sim$D), si12.P=b$p.D, si12.mean.sim=mean(b$sim$D), si12.se.sim=se(b$sim$D), si21.P=b2$p.D, si21.mean.sim=mean(b2$sim$D), si21.se.sim=se(b2$sim$D),an_breadth=NA,Niche_expansion=unname(filling.i$dynamic.index.w[1]),Niche_stability=unname(filling.i$dynamic.index.w[2]), Niche_unfilling=unname(filling.i$dynamic.index.w[3]))
...
resdsall$clade<-paste(Cdlist[8])

head(resdsall)
##### save resds
save(resdsall, file="Results/16.12.21.Results_2/resds_ecospat_realm_niche_clade8.Rdata")
write.table(resdsall,"Results/16.12.21.Results_2/resds_ecospat_realm_clade8.txt",sep='\t',quote=FALSE,row.names=F,col.names=T)
##### save simds
save(simdsall, file="Results/Clade8_Turdus/simds_ecospat_realm_clade8.Rdata")
